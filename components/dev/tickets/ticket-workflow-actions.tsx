...
export function TicketWorkflowActions(props: Props) {
  ...
  return (
    <div className="mt-5 border-t border-[#eceeef] pt-5">
      ...
      {action === "work" && <div><p className="mb-3 inline-flex items-center gap-2 text-sm font-medium text-[#187342]"><UserCheck aria-hidden="true" className="size-4" />Asignado a vos</p><div className="flex flex-col gap-3 sm:flex-row"><Link href={`/dev/projects/${props.projectId}/tickets/${props.ticketId}/workspace`} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-[#d9dadb] bg-white px-5 text-sm font-medium text-black hover:bg-[#f3f4f5]"><ArrowUpRight aria-hidden="true" className="size-4" />Abrir workspace</Link><button type="button" disabled={pending} onClick={() => run(() => sendTicketToReview(props.projectId, props.ticketId))} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-black px-5 text-sm font-medium text-white hover:bg-[#242424] disabled:cursor-wait disabled:opacity-60"><CheckCircle2 aria-hidden="true" className="size-4" />{pending ? "Enviando..." : "Enviar a revision"}</button></div></div>}

      {action === "waiting" && <p className="inline-flex items-center gap-2 text-sm font-medium text-[#8a5a00]"><Clock3 aria-hidden="true" className="size-4" />Entrega enviada. Esperando revision</p>}
