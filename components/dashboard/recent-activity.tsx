  ...
  return (
    <Card>
      <CardHeader className="border-b border-[#eceeef]">
        ...
        <CardDescription>Últimos movimientos en tus tickets y proyectos.</CardDescription>
      </CardHeader>
      <CardContent className="p-0 sm:p-0">
        {items.length === 0 ? (
          <div className="px-5 py-12 text-center">
            <p className="text-sm font-medium text-[#4c4546]">Todavía no hay actividad</p>
            <p className="mt-1 text-xs text-[#8a8b8c]">Las novedades de tu trabajo aparecerán acá.</p>
          </div>
        ) : (
                ...
                <li key={item.id} className="flex gap-3 px-5 py-4 transition-colors hover:bg-[#fafafa] sm:gap-4 sm:px-6">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <time className="shrink-0 text-xs text-[#8a8b8c]">{item.occurredAt}</time>
                    </div>
